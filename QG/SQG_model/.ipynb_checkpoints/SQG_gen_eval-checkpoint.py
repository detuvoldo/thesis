...
def main():
    ...
    parser.add_argument("--predict_folder", default=None, type=str, help="json for predictions.")
    ...
    ########################################################################
    with open(join(args.predict_file, "out_train.json")) as train_file:
        train_data = json.load(train_file)

    num = 0
    error_gen = 0
    res = []
    for ele in tqdm(train_data[:10]):
        #print(ele)
        tmp = {
            '_id': ele['_id'],
            'multihop_question': ele['question'],
            'sub_questions': []
        }
        context_1 = " ".join(ele['gold_para_1'])
        context_2 = " ".join(ele['gold_para_2'])
        bridge_answer = ele['bridge_ent']
        final_answer = ele['answer']
        
        if final_answer not in context_1:
            try:
                context = context_1
                question = ''
                answers = bridge_answer

                data = read_data(question = question, context = context, answer = answers)
                ...
                tmp['sub_questions'].append({"bridge_ent" : answers, "subquestion_1" : gen_Qs_sort[0]})
        ...
        try:
            ...
            tmp['sub_questions'].append({"final_answer" : answers, "subquestion_2" : gen_Qs_sort[0]})
            #print({"context" : context, "question" : question, "answers" : answers, "gen_question" : gen_Qs_sort})
    ...
    output_file = os.path.join(args.output_dir, "train_decomp.json")
    json.dump(res, open(output_file, "w"), indent = 4)
    
    ################################################################
    
    with open(args.predict_file) as dev_file:
    ...
    #print('num',num)
    print('error',error_gen)
    output_file = os.path.join(args.output_dir, "dev_decomp.json")
    json.dump(res, open(output_file, "w"), indent = 4)
